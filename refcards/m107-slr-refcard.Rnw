...
\title{Regression for Intro Stats}
...
\chead{ \bfseries \Large Regression in {\sf R} for Intro Stats}
\lhead{Math 107}
\rhead{Fall 2016}

This reference cards provides you with the recipes for working with regression models. You will need to carefully consider what ingredients need to be changed for each recipe.
